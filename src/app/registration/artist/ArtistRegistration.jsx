        ...
        >
          {currentStep > 1 && (
            <Col lg={4} md={6} sm={12} xs={12}>
              ...
              >
                بازگشت به مرحله قبل <i className="fas fa-angle-right" />
              </a>
            </Col>
          )}
...
class ArtistRegistration extends React.Component {
        ...
        if (error.response.status == 406) {
          this.setState({
            ...
            message: {
              ...
              message:
                "شما با این شماره یک هنرمند ثبت کرده اید.در صورت نیاز شما میتوانید با شماره‌ی دیگر ثبت نام کنید"
            },
            timer: 5
          });
        }
  ...
  redirectToApp = os => {
    ...
    axios
      .post(`${Urls().api()}/gallery-app/auth/get-token/`, body)
      .then(response => {
  ...
  addExhibition = async (pushFunction, type) => {
    axios
      .post(`${Urls().api()}/gallery-app/exb-art/create/`, {
        type: type
      })
      .then(response => {
  ...
  SingleEXBSubmit = (values, exbIndex) => {
    ...
    ExbValue.date = this.state.datePicker
    ...
    if (!SingleExbValidation(ExbValue)) {
    ...
    } else if (ExbValue.art_set == "" || ExbValue.art_set == undefined) {
      ...
      axios
        .post(`${Urls().api()}/gallery-app/artist/portfolio-step3/`, ExbValue)
        .then(response => {
  ...
  SingleArtSubmit = (values, Artindex, exbIndex) => {
    ...
    if (!SingleArtValidation(ArtValue)) {
      Toast(
        "warning",
        `لطفا تمام فیلدهای مربوط به اثر شماره ${Artindex + 1} را پر کنید`
      );
      ...
      axios
        .post(
          `${Urls().api()}/gallery-app/artist/portfolio-step3/art/`,
          ArtValue
        )
        .then(response => {
  ...
  handleRemoveExbArt = (FARemove, type, values, ExbIndex, ArtIndex) => {
    ...
    const ID =
      type === "exb_id"
        ? values.exb_set[ExbIndex].id
        : values.exb_set[ExbIndex].art_set[ArtIndex].id;
    // FARemove.remove(index)
    axios
      .delete(`${Urls().api()}/gallery-app/exb-art/create/?${type}=${ID}`)
      .then(response => {
  ...
  Step4_onChangeType = (portfolio_type, values) => {
    ...
    axios
      .post(`${Urls().api()}/gallery-app/artist/portfolio-step4/init/`, {
        type: portfolio_type
      })
      .then(response => {
        this.getSteps();
  ...
  Step4_addCollection = async (pushFunction, type) => {
    axios
      .post(`${Urls().api()}/gallery-app/collection-art/`, {
        type: type
      })
      .then(response => {
        pushFunction({ id: response.data.id, art_set: [] });
  ...
  importArttoCollection = (Artindex, ArtID, CollectionID) => {
    ...
    axios
      .post(`${Urls().api()}/gallery-app/artist/portfolio-step4/import/`, {
        art_id: ArtID,
  ...
  Step4_SingleArtSubmit = (values, Artindex, ColIndex, ArtType) => {
    ...
    if (!SingleCollectionArtValidation(ArtValue)) {
      ...
      Toast(
        "warning",
        `اطلاعات گالری مالک اثر برای اثر شماره ${Artindex + 1} را تکمیل نمایید`
      );
      ArtData.submitted = false;
      ArtValue.submitted = false;
      this.setState(
      ...
      axios
        .post(
          `${Urls().api()}/gallery-app/artist/portfolio-step4/art/`,
          ArtValue
        )
        .then(response => {
          Toast("success", `اثر شماره ${Artindex + 1} با موفقیت ثبت شد`);
  ...
  Step4_SingleColSubmit = (values, ColIndex) => {
    ...
    if (!SingleCollectionValidation(ColValue)) {
    ...
    } else if (ColValue.art_set == "" || ColValue.art_set == undefined) {
      ...
      axios
        .post(
          `${Urls().api()}/gallery-app/artist/portfolio-step4/collection/`,
          ColValue
        )
        .then(response => {
  ...
  Step4_handleRemove = (
    ...
    axios
      .delete(`${Urls().api()}/gallery-app/collection-art/?${type}=${ID}`)
      .then(response => {
        Toast("success", `${Name} مورد نظر با موفقیت حذف شد`);
        FARemove.remove(CurrentIndex);
      })
  ...
  render() {
    ...
    return (
      <>
        <RegisterMetaTags title={"ثبت نام"} />
        <Container>
          <Row>
            <Col xs={12}>
                  ...
                  <>
                    <Col xs={12}>
                      <Form
                        ...
                        initialValues={StepData !== "" ? StepData : null}
                        render={({
                          handleSubmit,
                          form,
                          submitting,
