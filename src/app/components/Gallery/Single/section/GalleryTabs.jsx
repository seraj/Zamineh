    ...
    <>
      ...
      {Data && Data.carousel_set && (
        <div className="carousel">
          <ShowSet item={Data.carousel_set} />
        </div>
      )}
      {Data && Data.about && (
...
export const Artists = ({ slug }) => {
  ...
  const handleData = () => {
    axios.get(`${Urls().api()}/gallery/${slug}/artists/`).then(({ data }) => {
      setData(data);
      setLoading(false);
    });
  };
...
export const Articles = ({ slug }) => {
  ...
  const handleData = page => {
    axios
      .get(`${Urls().api()}/gallery/${slug}/articles/`, {
        params: { page: page }
      })
      .then(({ data }) => {
        setData(data);
  ...
  return (
    <>
      ...
      {Data && Data.article_set && Data.article_set.length > 0 && (
        <div className={`${styles.Sections} nobb`}>
          <PaginationItem item={Data.article_set} type="article" />
          {Data && Data.page_count > 1 && (
            <Pagination
              pageCount={Data.page_count}
              onPageChange={handlePageClick}
...
export const Contact = ({ slug, title }) => {
  const [initialized, setInitialized] = useState(false);
  const [Data, setData] = useState();
  const [loading, setLoading] = useState(true);
  useEffect(() => {
  ...
  const handleData = page => {
    axios
      .get(`${Urls().api()}/gallery/${slug}/contact/`, {
        params: { page: page }
      })
      .then(({ data }) => {
        setData(data);
  ...
  return (
    <>
      ...
      {Data && (
        <div className={`${styles.Sections} ${styles.contact} nobb`}>
          {Data.email && (
            <div className="email">
              <a
                href={`mailto:${
                  Data.email
                }?subject=ارتباط با ${title} از زمینه`}
              >
                ارتباط با گالری {title} از طریق ایمیل
              </a>
            </div>
          )}
          {Data.address && (
