...
export const Ticket = () => {
    ...
    const getData = (page) => {
        axios.get(`${Urls().api()}/gallery-app/panel/support-tickets/`, {
            params: {
                page: page
            }
        })
    ...
    const getTypes = () => {
        axios.get(`${Urls().api()}/gallery-app/panel/support-types/`)
            .then(({ data }) => {
                setTypes(data)
            });
    }
        ...
        <React.Fragment>
            ...
            <section className={styles.tabSection}>
                ...
                <div className={styles.tabButton}>
                    <button className={`zbtn bradius black`} onClick={() => setModal(true)}>درخواست جدید</button>
                </div>
                {Data &&
                    Data.results.length > 0 ?
                    Data.results.map((item, index) => (
