...
const ArtOtherWorks = props => {
  return (
    <>
      {props.items.artist_other_works &&
        props.config.artist &&
        props.items.artist_other_works !== "" && (
          <Col xs={12} style={sectionStyle}>
            <div className={styles.workTitle}>
              <span>آثار دیگر {props.config.artist.name}</span>
              <Link
      ...
      {props.items.gallery_other_works &&
        props.config.gallery &&
        props.items.gallery_other_works !== "" && (
          <Col xs={12} style={sectionStyle}>
            <div className={styles.workTitle}>
              <span>آثار دیگر {props.config.gallery.name}</span>
              <Link
      ...
      {props.items.related_artist && props.items.related_artist !== "" && (
        <Col xs={12} style={sectionStyle}>
          <div className={styles.workTitle}>
            <span>هنرمندان مشابه</span>
          </div>
          <Row>
            {props.items.related_artist &&
              props.items.related_artist.map((item, index) => (
